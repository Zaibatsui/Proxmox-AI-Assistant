...
async def get_location_ssh_client(user_id: str, location: Optional[FileLocation] = None):
    ...
    if not location or location.type == "host":
        ...
        # Access LXC through Proxmox host using pct commands
        logger.info(f"Creating LXC wrapper for CT {location.id} through Proxmox host")
        
        # Get connection to Proxmox host
        host_client = await get_ssh_client(user_id)
        
        # Create wrapper that executes commands in LXC through pct
        class LXCCommandWrapper:
            def __init__(self, ssh_client, ctid):
        ...
        class VMCommandWrapper:
            ...
            def open_sftp(self):
                ...
                return VMSFTPWrapper(self._client, self._vmid)
        ...
        try:
            ...
            logger.info(f"Attempting SSH connection to VM {location.id} at {vm_ip}:{ssh_port} with user {location.ssh_username}")
            ssh_client = paramiko.SSHClient()
            ssh_client.set_missing_host_key_policy(paramiko.AutoAddPolicy())
            ssh_client.connect(
                vm_ip,
            ...
            logger.info(f"Successfully connected to VM {location.id} at {vm_ip}:{ssh_port}")
            # Mark this as a direct SSH connection so exec_in_location knows
            location.direct_ssh = True
            return ssh_client
        except paramiko.AuthenticationException as e:
            logger.error(f"SSH authentication failed for VM {location.id} at {vm_ip}:{ssh_port}: {str(e)}")
            raise HTTPException(status_code=401, detail=f"SSH authentication failed. Please check username and password.")
        except paramiko.SSHException as e:
            logger.error(f"SSH connection error for VM {location.id} at {vm_ip}:{ssh_port}: {str(e)}")
            raise HTTPException(status_code=500, detail=f"SSH connection error: {str(e)}")
        except socket.timeout as e:
            logger.error(f"SSH connection timeout for VM {location.id} at {vm_ip}:{ssh_port}")
            raise HTTPException(status_code=504, detail=f"Connection timeout. Cannot reach {vm_ip} on port {ssh_port}. Check if VM is running and SSH is enabled on that port.")
        except Exception as e:
            logger.error(f"Failed to connect to VM {location.id} at {vm_ip}:{ssh_port}: {str(e)}")
            raise HTTPException(status_code=500, detail=f"Failed to connect to VM via SSH at {vm_ip}:{ssh_port}: {str(e)}")
        ...
        if location.ssh_username and location.ssh_password:
            # Use provided ssh_host if available
            lxc_ip = getattr(location, 'ssh_host', None)
